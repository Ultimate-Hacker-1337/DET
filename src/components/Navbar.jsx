...
export default function Header() {
  ...
  return (
    <header className="bg-yellow-50 shadow-md border-b border-emerald-100 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        {/* Logo Section */}
        <Link to="/" className="flex items-center gap-3">
          <motion.img
        ...
        {/* Desktop Nav */}
        <nav
          className="hidden md:flex items-center gap-8 text-md font-semibold text-emerald-700"
          onMouseLeave={() => setOpenMenu(null)} // ✅ close when leaving nav
        >
          {navItems.map((item, i) => (
            <div
              key={i}
              className="relative inline-block" // ✅ avoid ghost hover zones
              onMouseEnter={() => setOpenMenu(i)}
            >
              <motion.div whileHover={{ scale: 1.05 }}>
                <Link
